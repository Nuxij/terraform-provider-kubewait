---
page_title: "kubewait_daemonsets Resource"
description: |-
  Waits for Kubernetes DaemonSets to meet specified conditions.
---

# kubewait_daemonsets Resource

Waits for Kubernetes DaemonSets to meet specified conditions before allowing dependent resources to proceed.

## Example Usage

```terraform
# Wait for daemonsets
resource "kubewait_daemonsets" "example" {
  namespace = "default"
  for       = "jsonpath='{.status.numberReady}'='{.status.desiredNumberScheduled}'"
  timeout   = 300
}
```
